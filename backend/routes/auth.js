...
router.post('/register', async (req, res) => {
  try {
    ...
    const artist = await Artist.create({ username, email, password: hashed, bio, profilePhoto, coverPhoto, mobileNumber });
    const token = jwt.sign({ id: artist._id }, process.env.JWT_SECRET, { expiresIn: '30d' });
    res.status(201).json({ success: true, token, artist: { _id: artist._id, username: artist.username, email: artist.email, bio: artist.bio, profilePhoto: artist.profilePhoto, coverPhoto: artist.coverPhoto, mobileNumber: artist.mobileNumber }});
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: artist._id }, process.env.JWT_SECRET, { expiresIn: '30d' });
    res.json({ success: true, token, artist: { _id: artist._id, username: artist.username, email: artist.email, bio: artist.bio, profilePhoto: artist.profilePhoto, coverPhoto: artist.coverPhoto, mobileNumber: artist.mobileNumber }});
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});

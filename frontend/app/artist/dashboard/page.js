    ...
    <main style={{padding:"20px", color:"white", background:"#080808", minHeight:"100vh", maxWidth:"680px", margin:"0 auto", paddingBottom:"40px"}}>
      ...
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"24px"}}>
        <div style={{display:"flex", alignItems:"center", gap:"12px"}}>
          ...
          <div>
            <h1 style={{fontWeight:"900", fontSize:"1.2rem"}}>{artist?.username || "Artist"}</h1>
            <p style={{color:"#555", fontSize:"0.75rem"}}>{artist?.email}</p>
          </div>
        </div>
        <button onClick={handleLogout} style={{padding:"8px 16px", borderRadius:"99px", background:"#1a1a1a", border:"1px solid #222", color:"#888", fontSize:"0.75rem", cursor:"pointer"}}>
          Sign Out
        </button>
      </div>
      ...
      <div style={{display:"flex", gap:"10px", marginBottom:"24px"}}>
        <button onClick={() => router.push("/upload")} style={{flex:1, padding:"14px", borderRadius:"12px", background:"#00e5ff", color:"#000", fontWeight:"800", fontSize:"1rem", border:"none", cursor:"pointer"}}>
          + Upload New Track
        </button>
        <button
          onClick={() => { const t = localStorage.getItem("token"); if(t) fetchMyTracks(t); }}
          style={{padding:"14px 16px", borderRadius:"12px", background:"#111", color:"#fff", fontWeight:"700", fontSize:"0.9rem", border:"1px solid #222", cursor:"pointer"}}
          title="Refresh stats"
        >
          🔄
        </button>
          ...
          <div key={track._id} style={{background:"#111", border:"1px solid #1e1e1e", borderRadius:"14px", padding:"14px 16px", marginBottom:"10px"}}>
            <div style={{display:"flex", alignItems:"center", gap:"12px", marginBottom:"12px"}}>
              ...
              <div style={{flex:1, minWidth:0}}>
                ...
                <p style={{color:"#555", fontSize:"0.75rem", marginTop:"2px"}}>{new Date(track.createdAt).toLocaleDateString()}</p>
              </div>
              <button onClick={() => setDeleteId(track._id)} style={{width:"36px", height:"36px", borderRadius:"50%", background:"rgba(255,71,87,0.1)", border:"1px solid rgba(255,71,87,0.3)", color:"#ff4757", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.9rem", flexShrink:0}}>
                🗑
              </button>
            </div>
      ...
      {deleteId && (
        <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:"20px"}}>
          <div style={{background:"#111", border:"1px solid #222", borderRadius:"16px", padding:"24px", maxWidth:"320px", width:"100%", textAlign:"center"}}>
            ...
            <p style={{color:"#555", fontSize:"0.82rem", marginBottom:"24px"}}>This cannot be undone.</p>
            <div style={{display:"flex", gap:"10px"}}>
              <button onClick={() => setDeleteId(null)} style={{flex:1, padding:"12px", borderRadius:"10px", background:"#1a1a1a", border:"1px solid #222", color:"#fff", fontWeight:"700", cursor:"pointer"}}>
                Cancel
              </button>
              <button onClick={() => handleDelete(deleteId)} style={{flex:1, padding:"12px", borderRadius:"10px", background:"#ff4757", border:"none", color:"#fff", fontWeight:"800", cursor:"pointer"}}>
                Delete
              </button>
            </div>
          </div>

  ...
  const pay = async () => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/gifts/promote`, {
        ...
        body: JSON.stringify({ trackId: track._id, phone: clean, days })
      });
      const data = await res.json();
      if (!data.success) { setError(data.error || "Payment failed."); setLoad(false); return; }
  ...
  const confirm = async () => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/gifts/promote/verify`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderTrackingId: orderRef, txRef, trackId: track._id, days })
      });
      const data = await res.json();
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:9999}}>
      <div style={{background:"linear-gradient(180deg,#1a0810,#0d0520)",border:"1px solid rgba(255,215,0,0.25)",borderRadius:"24px 24px 0 0",padding:"22px 20px",width:"100%",maxWidth:"480px",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {step === "form" && (
          <>
            <div style={{background:"rgba(255,215,0,0.06)",border:"1px solid rgba(255,215,0,0.15)",borderRadius:"10px",padding:"11px",marginBottom:"13px"}}>
              {["Pinned at top of home feed","Gold ⭐ Featured Artist badge","More plays, likes and gifts","From just UGX 1,000"].map(b => (
                <div key={b} style={{display:"flex",gap:"6px",marginBottom:"4px"}}>
                  <span style={{color:"#FFD700",fontSize:"0.68rem",flexShrink:0,marginTop:"1px"}}>✓</span>
                  <span style={{color:"#ddd",fontSize:"0.74rem"}}>{b}</span>
                </div>
              ))}
            </div>

            <p style={{fontSize:"0.67rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:"7px"}}>Choose Duration</p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:"6px",marginBottom:"11px"}}>
              {Object.entries(PRICES).map(([d,p]) => (
                <button key={d} onClick={() => setDays(Number(d))} style={{padding:"8px 4px",borderRadius:"8px",border:`2px solid ${days===Number(d)?"#FFD700":"rgba(255,255,255,0.1)"}`,background:days===Number(d)?"rgba(255,215,0,0.14)":"rgba(255,255,255,0.04)",cursor:"pointer",textAlign:"center"}}>
                  <p style={{color:days===Number(d)?"#FFD700":"#888",fontWeight:"800",fontSize:"0.76rem"}}>{d}d</p>
                  <p style={{color:"#aa7788",fontSize:"0.58rem",marginTop:"1px"}}>{p/1000}K</p>
                </button>
              ))}
            </div>
            ...
            <label style={{fontSize:"0.67rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",display:"block",marginBottom:"5px"}}>MTN / Airtel Number *</label>
            <input type="tel" placeholder="e.g. 0772000000" value={phone} onChange={e=>{setPhone(e.target.value);setError("");}} style={{padding:"12px 13px",borderRadius:"9px",border:`1px solid ${error?"#D4006A":"rgba(255,255,255,0.12)"}`,background:"rgba(255,255,255,0.06)",color:"#fff",fontSize:"0.86rem",width:"100%",outline:"none",marginBottom:"4px"}}/>
            <p style={{color:"#555",fontSize:"0.62rem",marginBottom:"12px"}}>Payment prompt sent to this number</p>

            {error && <div style={{background:"rgba(212,0,106,0.1)",border:"1px solid rgba(212,0,106,0.3)",borderRadius:"8px",padding:"8px 12px",marginBottom:"10px",color:"#ff6b9d",fontSize:"0.76rem"}}>⚠️ {error}</div>}

            <button onClick={pay} disabled={loading} style={{width:"100%",padding:"14px",borderRadius:"11px",background:loading?"#333":"linear-gradient(135deg,#FFD700,#E8640A)",color:loading?"#666":"#000",fontWeight:"800",fontSize:"0.93rem",border:"none",cursor:loading?"not-allowed":"pointer"}}>
        ...
        {step === "pending" && (
          <div style={{textAlign:"center",padding:"6px 0"}}>
            ...
            <p style={{color:"#aa7788",fontSize:"0.77rem",marginBottom:"14px",lineHeight:"1.6"}}>{msg}</p>
            {!isMock && (
            ...
            {error && <div style={{background:"rgba(212,0,106,0.1)",border:"1px solid rgba(212,0,106,0.3)",borderRadius:"8px",padding:"8px 12px",marginBottom:"10px",color:"#ff6b9d",fontSize:"0.76rem",textAlign:"left"}}>⚠️ {error}</div>}
            <button onClick={confirm} disabled={verify} style={{width:"100%",padding:"14px",borderRadius:"11px",background:verify?"#333":"linear-gradient(135deg,#FFD700,#E8640A)",color:verify?"#666":"#000",fontWeight:"800",fontSize:"0.93rem",border:"none",cursor:verify?"not-allowed":"pointer",marginBottom:"8px"}}>
              {verify ? "Checking..." : "✅ I Approved — Confirm"}
            </button>
            <button onClick={()=>setStep("form")} style={{width:"100%",padding:"10px",borderRadius:"11px",background:"transparent",color:"#888",fontWeight:"600",fontSize:"0.8rem",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer"}}>← Go Back</button>
          </div>
        )}
        ...
        {step === "success" && (
          <div style={{textAlign:"center",padding:"14px 0"}}>
            ...
            <p style={{color:"#aa7788",fontSize:"0.78rem",marginBottom:"20px",lineHeight:"1.6"}}>
              <strong style={{color:"#fff"}}>{track.title}</strong> appears at the top of the home feed.
            </p>
            <button onClick={onSuccess} style={{width:"100%",padding:"14px",borderRadius:"11px",background:"linear-gradient(135deg,#FFD700,#E8640A)",color:"#000",fontWeight:"800",fontSize:"0.93rem",border:"none",cursor:"pointer"}}>Done 🎉</button>
          </div>
        )}
      </div>
    </div>
...
export default function ArtistDashboard() {
  ...
  const fetchTracks = useCallback(async (t) => {
    ...
    if (!token) return;
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tracks/artist/mytracks`, {
      ...
      {tab === "tracks" && (
        loading ? (
          <div style={{textAlign:"center",padding:"30px 0"}}>
            ...
            <p style={{color:"#aa7788",fontSize:"0.8rem"}}>Loading tracks...</p>
          </div>
        ) : tracks.length === 0 ? (
      ...
      {tab === "gifts" && (
        gifts.length === 0 ? (
          <div style={{textAlign:"center",padding:"34px 0"}}>
            ...
            <p style={{color:"#aa7788",fontSize:"0.78rem"}}>Fans can gift you from your music cards</p>
          </div>
        ) : (
          gifts.map((g, i) => (

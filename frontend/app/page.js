  ...
  const handleSend = async () => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/gifts/send`,{
        ...
        body: JSON.stringify({ fromName:name||"Anonymous", toArtistId:track.artistId, toArtistName:track.artist, trackId:track._id, amount:useCard?0:amount, senderPhone:clean, message, giftCard:useCard?giftCard.toUpperCase():"" })
      });
...
export default function Home() {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tracks`)
      ...
      .then(data=>{
        ...
        setDisplayTracks(prev=>{
          if (!prev.length||!shuffled) return t;
  ...
  useEffect(()=>{ loadTracks(); const iv=setInterval(loadTracks,30000); return()=>clearInterval(iv); },[]);

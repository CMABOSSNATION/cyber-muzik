...
export default function Home() {
  ...
  // Wake backend - Render free tier sleeps after 15min, takes 30-50s to wake
  useEffect(() => {
    const api = process.env.NEXT_PUBLIC_API_URL;
    if (!api) return;
    fetch(api + '/').catch(() => {});
    setTimeout(() => fetch(api + '/').catch(() => {}), 6000);
    setTimeout(() => fetch(api + '/').catch(() => {}), 14000);
    setTimeout(() => fetch(api + '/').catch(() => {}), 22000);
  }, []);
  ...
  const loadTracks = useCallback((retry = 0) => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tracks`)
      ...
      .catch(() => {
        if (retry < 4) {
          setTimeout(() => loadTracks(retry + 1), 10000);
        } else {
          setTracks([]);
          setDisplayTracks([]);
        }

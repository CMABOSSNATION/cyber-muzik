    ...
    <main style={{padding:"20px", color:"white", background:"linear-gradient(160deg,#1a0810 0%,#0d0520 50%,#1a0810 100%)", minHeight:"100vh", maxWidth:"480px", margin:"0 auto", paddingBottom:"80px"}}>
      ...
      <div style={{marginBottom:"20px"}}>
        <div style={{display:"flex", alignItems:"center", background:"rgba(255,255,255,0.05)", borderRadius:"16px", border:`2px solid ${searchFocused ? "#E8640A" : "rgba(255,255,255,0.1)"}`, overflow:"hidden", transition:"all 0.3s", boxShadow: searchFocused ? "0 0 0 3px rgba(232,100,10,0.12)" : "none"}}>
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} placeholder="SEARCH TRACKS, ARTISTS..." style={{flex:1, height:"56px", background:"transparent", border:"none", outline:"none", color:"#fff", fontWeight:"700", fontSize:"0.92rem", padding:"0 14px"}}/>
          {searchQuery && <button onClick={() => setSearchQuery("")} style={{width:"38px", height:"38px", marginRight:"10px", borderRadius:"50%", background:"rgba(255,255,255,0.1)", border:"none", color:"#aaa", fontSize:"0.9rem", cursor:"pointer"}}>✕</button>}
        </div>
      </div>

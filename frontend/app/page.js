...
function GiftModal({ track, onClose }) {
  ...
  const handleSend = async () => {
    ...
    if (useCard && !giftCard) { setError("Enter your gift card code"); return; }
    setLoading(true);
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/gifts/send`,{
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ fromName:name||"Anonymous", toArtistId:track.artistId||track._id, toArtistName:track.artist, trackId:track._id, amount:useCard?0:amount, senderPhone:clean, message, giftCard:useCard?giftCard.toUpperCase():"" })
      });
  ...
  const handleVerify = async () => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/gifts/verify`,{
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ orderTrackingId:orderRef, txRef })
      });
      const data = await res.json();
      if (data.paid) { setStep("success"); }
      else { setError(data.message||"Not confirmed yet. Complete on your phone then try again."); }
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.93)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:1000}}>
      <div style={{background:"linear-gradient(180deg,#1a0810,#0d0520)",border:"1px solid rgba(232,100,10,0.3)",borderRadius:"24px 24px 0 0",padding:"24px 20px",width:"100%",maxWidth:"480px",maxHeight:"92vh",overflowY:"auto"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"16px"}}>
          <div>
            <h2 style={{fontWeight:"900",fontSize:"1.15rem",background:"linear-gradient(90deg,#E8640A,#D4006A)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>🎁 Gift {track.artist}</h2>
            <p style={{color:"#aa7788",fontSize:"0.7rem",marginTop:"2px"}}>Your name appears on their music after gifting ✨</p>
          </div>
        ...
        {step==="form" && (
          <>
            ...
            {!useCard ? (
              <>
                {/* Artist receives — platform fee hidden */}
                <div style={{background:"rgba(0,201,177,0.08)",border:"1px solid rgba(0,201,177,0.18)",borderRadius:"11px",padding:"13px",marginBottom:"14px",textAlign:"center"}}>
                  <p style={{color:"#aa7788",fontSize:"0.72rem",marginBottom:"3px"}}>Artist receives</p>
                  <p style={{color:"#00C9B1",fontWeight:"900",fontSize:"1.35rem"}}>UGX {artistGets.toLocaleString()}</p>
                  <p style={{color:"#aa7788",fontSize:"0.65rem",marginTop:"3px"}}>from your UGX {amount.toLocaleString()} gift</p>
                </div>
                <p style={{fontSize:"0.68rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:"7px"}}>Select Amount (UGX)</p>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"7px",marginBottom:"14px"}}>
                  {AMOUNTS.map(a=>(
                    <button key={a} onClick={()=>setAmount(a)} style={{padding:"9px",borderRadius:"9px",border:`2px solid ${amount===a?"#E8640A":"rgba(255,255,255,0.1)"}`,background:amount===a?"rgba(232,100,10,0.18)":"rgba(255,255,255,0.04)",color:amount===a?"#E8640A":"#888",fontWeight:"700",fontSize:"0.76rem",cursor:"pointer"}}>
                      {a>=1000?`${a/1000}K`:a}
                    </button>
                  ))}
                </div>
              ...
              <div style={{marginBottom:"14px"}}>
                <div style={{background:"rgba(255,215,0,0.07)",border:"1px solid rgba(255,215,0,0.18)",borderRadius:"11px",padding:"13px",marginBottom:"11px"}}>
                  <p style={{color:"#FFD700",fontWeight:"700",fontSize:"0.8rem",marginBottom:"8px"}}>🎫 Gift Cards:</p>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:"6px"}}>
                    {[{code:"CYBER100",val:"UGX 1,000"},{code:"CYBER500",val:"UGX 5,000"},{code:"CYBER1K",val:"UGX 10,000"},{code:"CYBERVIP",val:"UGX 50,000"}].map(c=>(
                      <div key={c.code} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,215,0,0.18)",borderRadius:"7px",padding:"7px",textAlign:"center"}}>
                        <p style={{color:"#FFD700",fontWeight:"800",fontSize:"0.7rem"}}>{c.code}</p>
                        <p style={{color:"#aa7788",fontSize:"0.62rem"}}>{c.val}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <label style={{fontSize:"0.68rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",display:"block",marginBottom:"5px"}}>Enter Code</label>
                <input type="text" placeholder="e.g. CYBER500" value={giftCard} onChange={e=>setGiftCard(e.target.value.toUpperCase())} style={{padding:"11px 13px",borderRadius:"9px",border:"1px solid rgba(255,215,0,0.28)",background:"rgba(255,255,255,0.05)",color:"#FFD700",fontSize:"0.88rem",width:"100%",outline:"none",fontWeight:"700",letterSpacing:"0.1em"}}/>
              </div>
            )}

            <div style={{display:"flex",flexDirection:"column",gap:"9px",marginBottom:"14px"}}>
              <div>
                <label style={{fontSize:"0.68rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",display:"block",marginBottom:"4px"}}>Your Name (shows on music)</label>
                <input type="text" placeholder="Anonymous" value={name} onChange={e=>setName(e.target.value)} style={{padding:"11px 13px",borderRadius:"9px",border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#fff",fontSize:"0.85rem",width:"100%",outline:"none"}}/>
              </div>
              <div>
                <label style={{fontSize:"0.68rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",display:"block",marginBottom:"4px"}}>MTN / Airtel Number *</label>
                <input type="tel" placeholder="e.g. 0772000000" value={phone} onChange={e=>{setPhone(e.target.value);setError("");}} style={{padding:"11px 13px",borderRadius:"9px",border:`1px solid ${error?"#D4006A":"rgba(255,255,255,0.1)"}`,background:"rgba(255,255,255,0.05)",color:"#fff",fontSize:"0.85rem",width:"100%",outline:"none"}}/>
                <p style={{color:"#555",fontSize:"0.62rem",marginTop:"3px"}}>A payment prompt will be sent to this number</p>
              </div>
              <div>
                <label style={{fontSize:"0.68rem",color:"#aa7788",textTransform:"uppercase",letterSpacing:"0.08em",display:"block",marginBottom:"4px"}}>Message (optional)</label>
                <input type="text" placeholder="Keep making great music! 🔥" value={message} onChange={e=>setMessage(e.target.value)} style={{padding:"11px 13px",borderRadius:"9px",border:"1px solid rgba(255,255,255,0.1)",background:"rgba(255,255,255,0.05)",color:"#fff",fontSize:"0.85rem",width:"100%",outline:"none"}}/>
              </div>
            </div>
            ...
            <button onClick={handleSend} disabled={loading} style={{width:"100%",padding:"15px",borderRadius:"12px",background:loading?"#333":"linear-gradient(135deg,#E8640A,#D4006A)",color:loading?"#666":"#fff",fontWeight:"800",fontSize:"0.95rem",border:"none",cursor:loading?"not-allowed":"pointer"}}>
        ...
        {step==="pending" && (
          <div style={{textAlign:"center",padding:"8px 0"}}>
            ...
            {!isMock && (
              <div style={{background:"rgba(232,100,10,0.07)",border:"1px solid rgba(232,100,10,0.18)",borderRadius:"11px",padding:"13px",marginBottom:"16px",textAlign:"left"}}>
                {["1. A payment prompt appears on your phone","2. Enter your PIN to approve the payment","3. Come back here and tap Confirm below"].map(s=>(
                  <div key={s} style={{display:"flex",gap:"6px",marginBottom:"5px",alignItems:"flex-start"}}>
                    <span style={{color:"#E8640A",fontSize:"0.68rem",marginTop:"2px"}}>→</span>
            ...
            <button onClick={handleVerify} disabled={verifying} style={{width:"100%",padding:"15px",borderRadius:"12px",background:verifying?"#333":"linear-gradient(135deg,#00C9B1,#006a5a)",color:verifying?"#666":"#fff",fontWeight:"800",fontSize:"0.95rem",border:"none",cursor:verifying?"not-allowed":"pointer",marginBottom:"8px"}}>
        ...
        {step==="success" && (
          <div style={{textAlign:"center",padding:"16px 0"}}>
            ...
            <p style={{color:"#aa7788",fontSize:"0.8rem",marginBottom:"22px",lineHeight:"1.6"}}>
              Your gift to <strong style={{color:"#fff"}}>{track.artist}</strong> was successful. Your name now appears on their music!
            </p>
            <button onClick={onClose} style={{width:"100%",padding:"15px",borderRadius:"12px",background:"linear-gradient(135deg,#00C9B1,#006a5a)",color:"#fff",fontWeight:"800",fontSize:"0.95rem",border:"none",cursor:"pointer"}}>Done ✓</button>
          </div>
        )}
      </div>
    </div>
...
function ArtistCard({ track, isActive, onPlay, index, onRefresh }) {
  ...
  useEffect(() => {
    ...
    const upd = () => setProgress((globalAudio.currentTime/globalAudio.duration)*100||0);
  ...
  const handleShare = async (e) => {
    ...
    if (navigator.share) { await navigator.share({title:track.title,text,url:window.location.href}); }
  ...
  return (
    <>
      ...
      <div onClick={onPlay} style={{position:"relative",borderRadius:"18px",overflow:"hidden",marginBottom:"14px",cursor:"pointer",background:gradients[index%gradients.length],border:`2px solid ${isPromoted?"#FFD700":isActive?"#E8640A":"rgba(255,255,255,0.07)"}`,transition:"all 0.2s",boxShadow:isPromoted?"0 0 20px rgba(255,215,0,0.18)":isActive?"0 0 22px rgba(232,100,10,0.22)":"none"}}>
        ...
        <div style={{position:"absolute",top:isPromoted?"38px":"12px",left:"12px",display:"flex",gap:"4px",flexWrap:"wrap"}}>
        ...
        {/* Stats badges */}
        <div style={{position:"absolute",top:isPromoted?"38px":"12px",right:"12px",display:"flex",gap:"4px",flexWrap:"wrap",justifyContent:"flex-end"}}>
          {isActive && (
            <div style={{background:"rgba(232,100,10,0.85)",backdropFilter:"blur(8px)",padding:"3px 9px",borderRadius:"99px",fontSize:"0.6rem",fontWeight:"700",display:"flex",alignItems:"center",gap:"3px"}}>
        ...
        <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"14px"}}>
          {gifters.length>0 && (
            <div style={{display:"flex",alignItems:"center",gap:"5px",marginBottom:"7px",flexWrap:"wrap"}}>
              ...
              {gifters.slice(-4).map((g,i)=>(
                <span key={i} style={{background:"rgba(255,215,0,0.14)",border:"1px solid rgba(255,215,0,0.28)",padding:"1px 7px",borderRadius:"99px",fontSize:"0.58rem",color:"#FFD700",fontWeight:"600"}}>{g.name}</span>
              ))}
            </div>
          )}
          <div style={{display:"flex",alignItems:"center",gap:"7px",marginBottom:"3px"}}>
            <p style={{fontWeight:"900",fontSize:"1.05rem"}}>{track.artist}</p>
          ...
          <p style={{color:"#ddd",fontSize:"0.76rem",marginBottom:"9px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>🎵 {track.title}</p>
...
export default function Home() {
  ...
  useEffect(()=>{ fetch(`${process.env.NEXT_PUBLIC_API_URL}/`).catch(()=>{}); },[]);
  ...
  const loadTracks = useCallback(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tracks`)
      ...
      .then(data=>{
        const t = Array.isArray(data?.data)?data.data:Array.isArray(data)?data:[];
        setTracks(t);
        setDisplayTracks(prev=>{
          if (!prev.length) return t;
          const shuffleActive = prev.some((p,i)=> i < t.length && p._id !== t[i]?._id);
          if (!shuffleActive) return t;
          return prev.map(p=>t.find(n=>n._id===p._id)||p);
        });
      })
      .catch(err=>{ console.error('loadTracks failed:', err); setTracks([]); setDisplayTracks([]); });
  },[]);

  useEffect(()=>{ loadTracks(); const iv=setInterval(loadTracks,30000); return()=>clearInterval(iv); },[loadTracks]);
  ...
  const filteredTracks = displayTracks.filter(t=>{
    const ms = !searchQuery||[t.title,t.artist,t.genre,t.country].some(f=>f?.toLowerCase().includes(searchQuery.toLowerCase()));
    const mg = activeGenre==="All"||t.genre===activeGenre;
    const mc = activeCountry==="All"||t.country===activeCountry;
    return ms&&mg&&mc;
  });
    ...
    <main style={{padding:"0",color:"white",background:"linear-gradient(160deg,#0a0a0f 0%,#0d0520 50%,#0a0a0f 100%)",minHeight:"100vh",maxWidth:"480px",margin:"0 auto",paddingBottom:"80px"}}>
      ...
      <div style={{padding:"18px 18px 0"}}>
        ...
        {trendingArtists.length>0 && (
          <div style={{display:"flex",gap:"11px",overflowX:"auto",paddingBottom:"14px"}}>
            {trendingArtists.map((t,i)=>(
              <div key={i} onClick={()=>setActiveTrack(t._id)} style={{flexShrink:0,width:"86px",cursor:"pointer",textAlign:"center"}}>
                <div style={{width:"86px",height:"86px",borderRadius:"13px",overflow:"hidden",marginBottom:"6px",border:t.promoted&&t.promotedUntil&&new Date(t.promotedUntil)>new Date()?"2px solid #FFD700":activeTrack===t._id?"2px solid #E8640A":"2px solid rgba(255,255,255,0.1)",position:"relative",background:"linear-gradient(135deg,#E8640A,#7B2FBE)"}}>
                  {(t.artistCover||t.artistPhoto||t.coverImage) ? <img src={t.artistCover||t.artistPhoto||t.coverImage} alt={t.artist} style={{width:"100%",height:"100%",objectFit:"cover"}}/> : <div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2rem"}}>🎤</div>}
                ...
                <p style={{fontSize:"0.62rem",fontWeight:"700",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.artist}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{marginBottom:"13px"}}>
          <div style={{display:"flex",alignItems:"center",background:"rgba(255,255,255,0.07)",borderRadius:"99px",border:`1px solid ${searchFocused?"#E8640A":"rgba(255,255,255,0.1)"}`,overflow:"hidden",padding:"0 15px",height:"46px",transition:"all 0.3s"}}>
            <span style={{fontSize:"0.95rem",marginRight:"9px",color:"#555"}}>🔍</span>
            <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} onFocus={()=>setSearchFocused(true)} onBlur={()=>setSearchFocused(false)} placeholder="Search tracks, artists, genres, countries..." style={{flex:1,height:"100%",background:"transparent",border:"none",outline:"none",color:"#fff",fontSize:"0.82rem"}}/>
            {searchQuery&&<button onClick={()=>setSearchQuery("")} style={{background:"none",border:"none",color:"#888",fontSize:"0.88rem",cursor:"pointer"}}>✕</button>}
          </div>
        </div>
      </div>
      ...
      <div style={{paddingLeft:"18px",marginBottom:"7px"}}>
        <div style={{display:"flex",gap:"7px",overflowX:"auto",paddingBottom:"7px",paddingRight:"18px"}}>
          {GENRES.map(g=>(
            <button key={g} onClick={()=>setActiveGenre(g)} style={{flexShrink:0,padding:"5px 13px",borderRadius:"99px",border:"none",cursor:"pointer",fontSize:"0.7rem",fontWeight:"700",background:activeGenre===g?"linear-gradient(135deg,#E8640A,#D4006A)":"rgba(255,255,255,0.07)",color:activeGenre===g?"#fff":"#888",transition:"all 0.2s"}}>
              {g}
            </button>
          ))}
        </div>
      ...
      <div style={{padding:"0 18px",marginBottom:"14px"}}>
        <button onClick={()=>setShowCountries(!showCountries)} style={{display:"flex",alignItems:"center",gap:"5px",padding:"5px 13px",borderRadius:"99px",background:activeCountry!=="All"?"linear-gradient(135deg,#FFD700,#E8640A)":"rgba(255,255,255,0.07)",color:activeCountry!=="All"?"#000":"#888",border:"none",cursor:"pointer",fontSize:"0.7rem",fontWeight:"700"}}>
        ...
        {showCountries && (
          <div style={{display:"flex",gap:"6px",flexWrap:"wrap",marginTop:"9px"}}>
            {COUNTRIES.map(c=>(
              <button key={c} onClick={()=>{setActiveCountry(c);setShowCountries(false);}} style={{padding:"4px 11px",borderRadius:"99px",border:"none",cursor:"pointer",fontSize:"0.68rem",fontWeight:"700",background:activeCountry===c?"linear-gradient(135deg,#FFD700,#E8640A)":"rgba(255,255,255,0.07)",color:activeCountry===c?"#000":"#888"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div style={{padding:"0 18px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}>
          ...
          <div style={{display:"flex",gap:"7px"}}>
            {(activeGenre!=="All"||activeCountry!=="All")&&<button onClick={()=>{setActiveGenre("All");setActiveCountry("All");}} style={{padding:"4px 9px",borderRadius:"99px",background:"rgba(255,71,87,0.14)",color:"#ff4757",border:"1px solid rgba(255,71,87,0.28)",cursor:"pointer",fontSize:"0.62rem",fontWeight:"700"}}>✕ Clear</button>}
        ...
        {tracks===null ? (
          <div style={{textAlign:"center",padding:"40px 0"}}><p style={{color:"#aa7788"}}>Loading tracks...</p></div>
        ) : filteredTracks.length>0 ? (
          <>
            {filteredTracks.map((t,i)=>(
              <ArtistCard key={t._id} track={t} index={i} isActive={activeTrack===t._id} onPlay={()=>setActiveTrack(activeTrack===t._id?null:t._id)} onRefresh={loadTracks}/>
            ))}
            ...
            {!searchQuery&&activeGenre==="All"&&activeCountry==="All"&&top10.length>0&&(
              <div style={{marginTop:"22px",marginBottom:"22px"}}>
                ...
                {top10.map((t,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:"11px",padding:"9px 13px",borderRadius:"9px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",marginBottom:"7px",cursor:"pointer"}} onClick={()=>setActiveTrack(t._id)}>
                    ...
                    <div style={{width:"40px",height:"40px",borderRadius:"7px",overflow:"hidden",flexShrink:0,background:"rgba(255,255,255,0.1)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {(t.artistCover||t.artistPhoto||t.coverImage)?<img src={t.artistCover||t.artistPhoto||t.coverImage} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"🎵"}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontWeight:"700",fontSize:"0.85rem",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{t.title}</p>
                      <p style={{color:"#aa7788",fontSize:"0.68rem"}}>{t.artist}{t.genre&&<span style={{color:"#00C9B1"}}> · {t.genre}</span>}{t.country&&<span style={{color:"#FFD700"}}> · {t.country}</span>}</p>
                    </div>
              ...
              <div style={{marginBottom:"22px"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"9px"}}>
                  {[{n:"Hip Hop",i:"🎤",c:"#E8640A"},{n:"Afrobeats",i:"🌍",c:"#D4006A"},{n:"Electronic",i:"🎛️",c:"#7B2FBE"},{n:"K-Pop",i:"🎵",c:"#00C9B1"},{n:"Dancehall",i:"🎶",c:"#FFD700"},{n:"R&B",i:"💜",c:"#D4006A"},{n:"Gospel",i:"✝️",c:"#E8640A"},{n:"Reggae",i:"🌿",c:"#00C9B1"}].map(g=>(
                    <button key={g.n} onClick={()=>setActiveGenre(g.n)} style={{padding:"12px 6px",borderRadius:"11px",background:"rgba(255,255,255,0.05)",border:`1px solid ${activeGenre===g.n?g.c:"rgba(255,255,255,0.07)"}`,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:"5px"}}>
          ...
          <div style={{textAlign:"center",padding:"38px 0"}}>
            ...
            <button onClick={()=>{setSearchQuery("");setActiveGenre("All");setActiveCountry("All");}} style={{marginTop:"13px",padding:"8px 20px",borderRadius:"99px",background:"linear-gradient(135deg,#E8640A,#D4006A)",color:"#fff",border:"none",cursor:"pointer",fontSize:"0.78rem",fontWeight:"700"}}>Clear Filters</button>
          </div>
        )}
        ...
        <div style={{marginTop:"22px",padding:"18px",borderRadius:"14px",background:"linear-gradient(135deg,rgba(255,215,0,0.07),rgba(232,100,10,0.07))",border:"1px solid rgba(255,215,0,0.18)",textAlign:"center",marginBottom:"12px"}}>
          ...
          <a href="/artist/dashboard" style={{padding:"9px 18px",borderRadius:"99px",background:"linear-gradient(135deg,#FFD700,#E8640A)",color:"#000",fontWeight:"800",fontSize:"0.8rem",textDecoration:"none"}}>Promote My Music ⭐</a>
        </div>

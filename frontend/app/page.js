            ...
            {!useCard ? (
              <>
                <div style={{background:"rgba(0,201,177,0.08)",border:"1px solid rgba(0,201,177,0.18)",borderRadius:"11px",padding:"12px",marginBottom:"13px",textAlign:"center"}}>
                  <p style={{color:"#aa7788",fontSize:"0.7rem",marginBottom:"3px"}}>Artist receives</p>
                  <p style={{color:"#00C9B1",fontWeight:"900",fontSize:"1.3rem"}}>UGX {artistGets.toLocaleString()}</p>
                  <p style={{color:"#aa7788",fontSize:"0.62rem",marginTop:"3px"}}>from your UGX {amount.toLocaleString()} gift</p>
                </div>
...
export default function Home() {
  ...
  // Wake backend on load - ping multiple times since Render free tier sleeps
  useEffect(() => {
    const api = process.env.NEXT_PUBLIC_API_URL;
    fetch(`${api}/`).catch(() => {});
    setTimeout(() => fetch(`${api}/`).catch(() => {}), 5000);
    setTimeout(() => fetch(`${api}/`).catch(() => {}), 12000);
  }, []);

  const loadTracks = useCallback((retry = 0) => {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/tracks`, { signal: controller.signal })
      ...
      .then(data => {
        ...
        setDisplayTracks(prev => {
          ...
          const isShuffled = prev.some((p, i) => p._id !== (t[i]?._id));
      ...
      .catch(err => {
        ...
        console.error("Load tracks failed:", err.message);
        // Retry up to 3 times with delay (backend may be waking up)
        if (retry < 3) {

...
async function uploadImageToSupabase(file) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    throw new Error("Supabase not configured. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to Vercel env vars.");
  }
...
// ─── Upload audio to Cloudinary with XHR progress bar ────────────────────────
function uploadAudioToCloudinary(file, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    if (!cloudName || !uploadPreset) {
      reject(new Error("Cloudinary not configured. Add NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME and NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET to Vercel env vars."));
      return;
    }
    ...
    // eager: create a streaming-friendly version so playback starts in 2-5s
    fd.append("eager", "fl_streaming_attachment,q_auto");
    ...
    xhr.onload = () => {
      try {
        ...
        if (xhr.status === 200) {
          // Prefer the eager (streaming) URL if available, else fallback to secure_url
          const streamingUrl = result.eager?.[0]?.secure_url || result.secure_url;
...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Step 1 — cover image via Supabase (FREE, no Cloudinary credits used)
      if (coverFile) {
        setUploadStep("cover");
        setCoverPct(10); // Show progress started
        coverImage = await uploadImageToSupabase(coverFile);
        setCoverPct(100);
      }

      // Step 2 — audio via Cloudinary (streaming-optimised)
      setUploadStep("audio");
      setAudioPct(0);
      const audioUrl = await uploadAudioToCloudinary(file, setAudioPct);
      ...
      if (!data.success) {
        setMessage(data.error || "Track saved but failed to record in database.");
        setLoading(false);
        setUploadStep("");
        return;
      }
    ...
    <main style={{padding:"20px",color:"white",background:"linear-gradient(160deg,#0a0a0f,#0d0520,#0a0a0f)",minHeight:"100vh",maxWidth:"480px",margin:"0 auto",paddingBottom:"40px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
        ...
        <div style={{background:"rgba(0,201,177,0.06)",border:"1px solid rgba(0,201,177,0.2)",borderRadius:"10px",padding:"10px 12px",display:"flex",alignItems:"center",gap:"8px"}}>
          ...
          <p style={{color:"#00C9B1",fontSize:"0.72rem",lineHeight:"1.4"}}>
            <strong>Cover images</strong> use free Supabase Storage. Only audio uses Cloudinary credits.
          </p>
        </div>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(232,100,10,0.2)",borderRadius:"12px",padding:"16px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"14px"}}>
              ...
              <p style={{fontWeight:"700",fontSize:"0.82rem",color:"#E8640A"}}>
                {uploadStep==="cover" ? "📸 Uploading cover (Supabase — free)..."
                  : uploadStep==="audio" ? "🎵 Uploading audio (streaming mode)..."
          ...
          <div style={{background:message.includes("🎉")?"rgba(0,201,177,0.08)":"rgba(212,0,106,0.1)",border:`1px solid ${message.includes("🎉")?"rgba(0,201,177,0.3)":"rgba(212,0,106,0.3)"}`,borderRadius:"10px",padding:"12px 14px"}}>
            ...
            {message.includes("Supabase not configured") && (
              <p style={{color:"#aa7788",fontSize:"0.75rem",marginTop:"6px"}}>
                Go to Vercel → your project → Settings → Environment Variables and add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY, then redeploy.
              </p>
            )}
